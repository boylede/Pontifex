...
module.exports.loop = function () {
    ...
    if(spawn.spawning) { 
       ...
       var err = spawn.createCreep(stageC.fatHarvester, undefined, {role: 'fatHarvester'});
       if (err != OK) {
           s.structErr(spawn, err);
           console.log('error while spawning fatHarvester.');
       };
    } else if (sherpas.length < stageC.desiredSherpas) {
        var err = spawn.createCreep(stageC.sherpa, undefined, {role: 'sherpa'});
        if (err != OK) {
           s.structErr(spawn, err);
